...
for(var i=0;i<Object.keys(basicGraph).length;i++){
	...
	pointA = new L.LatLng(nodes[basicGraph[i]["s"]].coord[0],nodes[basicGraph[i]["s"]].coord[1]);
	pointB = new L.LatLng(nodes[basicGraph[i]["f"]].coord[0],nodes[basicGraph[i]["f"]].coord[1]);
	//var pointList = [pointA, pointB];
	pointListConnections.push(new L.Polyline([pointA, pointB], {
		color: 'blue',
		weight: 3,
		opacity: 0.5,
		smoothFactor: 1
	...
	if(parseInt(basicGraph[i]["t"]) == 1){
		pointListConnections.push(L.polylineDecorator([pointA, pointB], {
		patterns: [
		  {offset: 0, repeat: 25, symbol: L.Symbol.arrowHead({pixelSize: 8, polygon: false, pathOptions: {stroke: true}})}
		]
		}).addTo(map));
	}
...
//var closestPt = L.GeometryUtil.closestLayer(map, [], [14.5640756, 121.0535951]);
//closestPt.click();
//console.log(JSON.stringify(closestPt));
/*
...
$.getJSON('https://cdn.rawgit.com/johan/world.geo.json/34c96bba/countries/GBR.geo.json').then(function(geoJSON) {
  var osm = new L.TileLayer.BoundaryCanvas("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png", {
    boundary: geoJSON,
    attribution: '&copy; <b href="http://osm.org/copyright">OpenStreetMap</b> contributors, UK shape <b href="https://github.com/johan/world.geo.json">johan/word.geo.json</b>'
  });
  map.addLayer(osm);
...
map.on('click', function(e){
	if($("#actionToDo").val()==1){
		...
		let ptag = parseInt(prompt("Tag (0 = Transpo, 1 = Resto, 2 = Hotel, 3 = Shop, 4 = Airport, 5 = None)", "5"));
		if(pname != "" && ptag != ""){
			nodes.push({coord:[e.latlng["lat"],e.latlng["lng"]], name: pname, tag: ptag});
			//alert(JSON.stringify( ));
			L.marker(e.latlng,{pane: 'nodes',title: Object.keys(nodes)[Object.keys(nodes).length-1],icon:iconsAr[ptag]}).bindTooltip((Object.keys(nodes).length-1)+"", {permanent: true, className: "my-label", offset: [0, 0] }).bindPopup(pname).addTo(map).on("click", circleClick);
...
function addDirection(index){
	...
	if(newDirection.length >= 2){
		let pType = parseInt(prompt("Type (1 = One Way, 2 = Two Way)", "2"));
		if(pType == 2 || pType == 1)
		if(confirm(nodes[newDirection[0]].name + " -> " +  nodes[newDirection[1]].name + " (" +(pType == 2 ? "Two Way":"One Way")+")")){
			//if(!graph[newDirection[0]][newDirection[1]]){
				basicGraph.push({s: newDirection[0],f: newDirection[1], t: (pType == 2 ? 2:1)});
				//graph = readyGraph(basicGraph);
				
				pointA = new L.LatLng(nodes[newDirection[0]].coord[0],nodes[newDirection[0]].coord[1]);
				pointB = new L.LatLng(nodes[newDirection[1]].coord[0],nodes[newDirection[1]].coord[1]);
				pointListConnections.push(new L.Polyline([pointA, pointB], {
					color: 'blue',
					weight: 3,
					opacity: 0.5,
...
//(0 = Transpo, 1 = Resto, 2 = Hotel, 3 = Shop, 4 = Airport, 5 = None)
const iconsAr = [
L.icon({
  iconSize: [25, 25],
  iconAnchor: [10, 20],
...
function showNodes(nodes){
  for(var a in nodes){
    ...
    markerGroupLayer.addLayer(L.marker(nodes[a].coord,{pane: 'nodes',title: a,icon:iconsAr[nodes[a].tag]})
...
function showPosition(position) {
	...
	var closestMarker = L.GeometryUtil.closestLayer(map, markerGroupLayer.getLayers(), [position.coords.latitude,position.coords.longitude]);
	closestMarker.layer.openPopup();
	//console.log(closestMarker);
	map.setView(closestMarker.latlng , 15);
	
	L.marker([position.coords.latitude,position.coords.longitude],{pane: 'nodes',title: "Current Location",icon:cIcon}).addTo(map);
...
<table class="table text-white">
	...
	<h5>Path</h5> <ul id="pRoute" class="timeline"> </ul>`);
		...
		for(var i=1;i<path.length;i++){
			...
			if(toTest(sIndex,fIndex)){
				if (tempWPath.length >= 1) {
					...
					tempText += `<li>
									<b target="_blank" href="#">Walk</b>
									<b href="#" class="float-right">`+showTime(time)+`</b>
									...
									<div class="row">
										...
										<div class="col-4">
										  <b href="#" class="float-right">`+showDistance(distance)+`</b>
										</div>
									</div>
								</li>`;
					//alert(tempWPath + " 0");
					showPath(tempWPath,1);
					//alert(tempWPath + " 01");
				...
				if(routeTemp[sIndex][fIndex]["name"] == tempRoutePath["name"]){
					...
					tempRoutePath["finish"] = nodes[tempRPat[tempRPat.length - 1]].name;
					tempRoutePath["distance"] += distance;
					tempRoutePath["time"] += time;
					tempRoutePath["route"].pop();
					tempRPat.forEach(function (item, index) {
						tempRoutePath["route"].push(item);
					});
				}
				else{
					if(tempRoutePath["name"] !== ""){
						...
						tempText += `<li>
										<b target="_blank" href="#">`+tempRoutePath["name"]+`</b>
										<b href="#" class="float-right">`+showTime(tempRoutePath["time"])+`</b>
										...
										<div class="row">
											...
											<div class="col-4">
											  <b href="#" class="float-right">`+showDistance(tempRoutePath["distance"])+`</b>
											</div>
										</div>
									</li>`;
						//alert(tempRoutePath["route"] + " 1");
						showPath(tempRoutePath["route"],2);
					}
				...
				if(i == path.length-1){
					if (tempWPath.length > 1) {
						...
						//alert([sIndex,fIndex] + " 4");
						showPath([sIndex,fIndex],1);
					}
				}
				//console.log(sIndex);
			}
			if(i == path.length-1 ){
				finalizedPath.push(path[path.length - 1]);
...
//}
//var polyTemp;
function showPath(path, type){
	var lineCoords = [];
	lineCoords.push( nodes[path[0]].coord);
	for(var i=1;i<path.length;i++){
		var nodeName =path[i];
	...
	*/
	...
	if(type == 1){
		polylinePath.push(L.polyline(lineCoords, {color: 'black',weight: 6,dashArray: '5,10'}).addTo(map));
		//polylinePath.push(polyTemp);
	}
	else{
		polylinePath.push(L.polyline(lineCoords, {color: 'black',weight: 6}).addTo(map));
		//polylinePath.push(polyTemp);
		
		polylinePath.push(L.polyline(lineCoords, {color: 'white',weight: 3,dashArray: '5,10'}).addTo(map));
		//polylinePath.push(polyTemp);
...
function clearMap() {
	if(polylinePath.length >= 1){
		polylinePath.forEach(function (item, index) {
		...
		while (true) {
			...
			if(lowestPriorityIndex == f){
				...
				for(var i = path.length-1; i > 0; i--){
					...
					if(!graph[ path[i-1] ][ path[lastValid] ]){
						toRemove.push(i-1);
						//console.log("remove this "   +path[i-1]+ " + " + path[lastValid]);
					}
					else {
						if(i>=2 && graph[ path[i-2] ][  path[lastValid] ]){
							//if(i>=3 && !graph[path[i-3]][path[lastValid]]){
								//lastValid = i-2;
								//console.log("valid1");
							//}
							//else{
